...
export function assertTitle($: Doc, expected: string) {
	assert.equal($('title').text().trim(), expected, `<title> mismatch`);
}

/** Assert `<title>` text includes a substring. */
export function assertTitleIncludes($: Doc, substring: string) {
...
export function assertExists($: Doc, selector: string) {
	assert.ok(
		$(selector).length > 0,
...
export function assertAttr(
	$: Doc,
	selector: string,
	attr: string,
	expected: string,
) {
	const actual = $(selector).first().attr(attr);
	assert.equal(actual, expected, `"${selector}" attr "${attr}" mismatch`);
}
