...
import { findDefinedCustomElement } from './custom-elements-define.js';
import { findDecoratedCustomElement } from './decorators.js';
import {
	injectRegisterClass,
	injectRuntime,
	isClassRegistered,
...
import { parseOptions, singlePath, addToSet } from './utils.js';
import { isFunctionComponent } from './functions.js';
import { getImportedVariableNames } from './get-imported-variable-names.js';
import { implementsBaseClass } from './class.js';

/** @returns {PluginObj} */
export default function babelPluginWcHmr() {
	return {
		visitor: {
			Program(program) {
				...
				const options = parseOptions(
					/** @type {BabelPluginWcHmrOptions} */ (this.opts),
				);
				const baseClasses = options.baseClasses || [];
				...
				program.traverse({
					CallExpression(callExpr) {
						if (
							...
							isFunctionComponent(callExpr, functionNames)
						) {
						...
						const arguments_ = callExpr.get('arguments');
						if (!singlePath(callee) || !Array.isArray(arguments_)) {
							return;
						}

						if (callee.isMemberExpression()) {
							// this might be a customElements.define call
							const definedCustomElement = findDefinedCustomElement(
								callee,
								arguments_,
							);
						...
						if (decoratorNames.size > 0 && callee.isIdentifier()) {
					...
					ClassExpression(classExpr) {
						const { parent, parentPath: variableDeclarator } = classExpr;
						if (!parent || !variableDeclarator.isVariableDeclarator()) {
							return;
						}
						if (
							!variableDeclarator ||
						...
						const injectScope = variableDeclarator.parentPath;
						if (!injectScope) {
							return;
						}
						...
						if (
							classExpr.isExpression() &&
							implementsBaseClass(classExpr, baseClassNames)
						) {
