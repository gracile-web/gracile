...
globalThis.customElements.define = (name, ...rest) => {
	if (!globalThis.customElements.get(name)) {
		originalDefine.call(globalThis.customElements, name, ...rest);
	}
};
...
function trackConnectedElements(hmrClass) {
	...
	const originalCallback = hmrClass.prototype.connectedCallback;
	hmrClass.prototype.connectedCallback = function connectedCallback(
		...arguments_
	) {
		if (originalCallback) {
			originalCallback.call(this, ...arguments_);
		}
	...
	hmrClass.prototype.disconnectedCallback = function disconnectedCallback(
		...
		if (originalDcb) {
			originalDcb.call(this, ...arguments_);
		}
...
function createProxy(originalTarget, getCurrentTarget) {
	...
	for (const method of proxyMethods) {
		proxyHandler[method] = (_, ...arguments_) => {
			if (method === 'get' && arguments_[0] === 'prototype') {
				// prototype must always return original target value
				return Reflect[method](_, ...arguments_);
			}
			return Reflect[method](getCurrentTarget(), ...arguments_);
		};
	}
...
globalThis.WebComponentHmr = WebComponentHmr;
