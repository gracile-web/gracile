 ...
 *
 * Uses HTTP polling for readiness detection — much more reliable than
 * watching stdout events.
 */
...
export async function launchServer(
	...
	entryFile: string,
	port: number,
	options?: { timeoutMs?: number; hostname?: string; fixturesDir?: string },
): Promise<ServerProcess> {
	const fixturesDirectory = options?.fixturesDir ?? resolveFixtures();
	const cwd = join(fixturesDirectory, fixture);
